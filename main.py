...
def http_request(url, api_key, api_secret):
    ...
    try:
        ...
        response = requests.get(
            url, auth=HTTPBasicAuth(api_key, api_secret), verify=False
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--opnsense-ip",
        required=True,
        help="OPNsense router IP or URL (e.g., https://192.168.1.1)",
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Cloudflare DNS record names (not including domain name)",
    )
    ...
    for record_name in args.record_name:
        record_id, cloudflare_ip = get_cloudflare_dns_ip(
            args.zone_id, record_name, args.api_token
        )
        ...
        if public_ip != cloudflare_ip:
            logger.info(
                f"IP mismatch detected for {record_name}. Updating Cloudflare ({cloudflare_ip} -> {public_ip})"
            )
            update_cloudflare_dns(
                args.zone_id, record_id, record_name, public_ip, args.api_token
            )
        else:
            logger.info(
                f"Cloudflare DNS record for {record_name} is already up to date. No update needed."
            )
